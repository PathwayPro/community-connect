...
const kAttachmentModes = ['local', 'missing'];
...
async function mergeReports(reportDir, opts) {
  ...
  if (!(await _fs.default.promises.stat(dir)).isDirectory()) throw new Error('Directory does not exist: ' + dir);
  ...
  if (opts.attachments) {
    if (!kAttachmentModes.includes(opts.attachments)) throw new Error(`Invalid --attachments value "${opts.attachments}", must be one of ${kAttachmentModes.map(name => `"${name}"`).join(', ')}.`);
  }
  const resolveAttachmentPaths = opts.attachments !== 'missing';
  await (0, _merge.createMergedReport)(config, dir, reporterDescriptions, resolveAttachmentPaths);
}
