...
class UIMode {
  ...
  async showUI(options) {
    ...
    await (0, _server.showTraceViewer)([], 'chromium', {
      app: 'uiMode.html',
  ...
  async _listTests() {
    const listReporter = new _teleEmitter.TeleReporterEmitter(e => this._dispatchEvent(e.method, e.params));
    const reporter = new _internalReporter.InternalReporter([listReporter]);
  ...
  async _runTests(testIds) {
    ...
    reporters.push(new _teleEmitter.TeleReporterEmitter(e => this._dispatchEvent(e.method, e.params)));
