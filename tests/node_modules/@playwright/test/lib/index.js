...
class ArtifactsRecorder {
  ...
  async didFinishTest() {
      ...
      if (captureScreenshots) {
        await Promise.all(context.pages().map(async page => {
          ...
          await page.screenshot({
            ...
            timeout: 5000,
            path: this._addScreenshotAttachment(),
            caret: 'initial'
          }).catch(() => {});
        }));
      }
    ...
    await Promise.all(this._temporaryScreenshots.map(async file => {
      if (captureScreenshots) await fs.promises.rename(file, this._addScreenshotAttachment()).catch(() => {});else await fs.promises.unlink(file).catch(() => {});
    }));
