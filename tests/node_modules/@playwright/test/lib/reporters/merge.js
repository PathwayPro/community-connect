...
async function createMergedReport(config, dir, reporterDescriptions, resolvePaths) {
  ...
  const receiver = new _teleReceiver.TeleReporterReceiver(_path.default.sep, new _multiplexer.Multiplexer(reporters), false, config.config);
...
async function extractReportFromZip(file) {
  const zipFile = new _utils.ZipFile(file);
  const entryNames = await zipFile.entries();
  try {
    for (const entryName of entryNames) {
      if (entryName.endsWith('.jsonl')) return await zipFile.read(entryName);
    }
  } finally {
    zipFile.close();
  }
...
async function mergeEvents(dir, shardReportFiles) {
  ...
  for (const reportFile of shardReportFiles) {
    const reportJsonl = await extractReportFromZip(_path.default.join(dir, reportFile));
    const parsedEvents = parseEvents(reportJsonl);
    for (const event of parsedEvents) {
      if (event.method === 'onConfigure') configureEvents.push(event);else if (event.method === 'onBegin') beginEvents.push(event);else if (event.method === 'onEnd') endEvents.push(event);else if (event.method === 'onBlobReportMetadata') new ProjectNamePatcher(event.params.projectSuffix).patchEvents(parsedEvents);else events.push(event);
    }
  }
  return [mergeConfigureEvents(configureEvents), mergeBeginEvents(beginEvents), ...events, mergeEndEvents(endEvents), {
    method: 'onExit',
    params: undefined
  }];
}
